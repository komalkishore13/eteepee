    ...
    <div className="h-full w-full bg-[#0f0f0f] flex items-center justify-center overflow-hidden">
      <div className="flex items-end gap-4 h-full py-4 px-2">
        ...
        >
          ...
          {/* animated video slot */}
          <div className={`absolute inset-0 flex items-center justify-center ${animClass}`}>
            <iframe
              key={`${current.youtubeId}-${displayIndex}`}
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[420px] bg-[#212121] flex flex-col z-50 transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <span className="text-white font-semibold text-lg">
            Comments{" "}
            <span className="text-white/50 font-normal text-base ml-1">
              {current.comments.toLocaleString()}
            </span>
          </span>
        ...
        <div className="flex items-center gap-3 px-6 py-3 border-b border-white/10">
          <div className="w-8 h-8 rounded-full bg-[#3d3d3d] flex items-center justify-center text-white text-xs font-bold shrink-0">
            U
          </div>
          <input
            type="text"
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  <button className="flex items-center gap-1 text-white/50 hover:text-white text-xs transition-colors">
                    <ThumbsUp size={13} /> {c.likes.toLocaleString()}
                  </button>
                  <button className="flex items-center gap-1 text-white/50 hover:text-white text-xs transition-colors">
                    <ThumbsDown size={13} />
                  </button>
      ...
      {showShare && (
        <>
          ...
          />
          <div className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-[#282828] rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <span className="text-white font-semibold text-base">Share</span>
              <button onClick={() => setShowShare(false)} className="text-white/60 hover:text-white">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex gap-4 justify-center mb-6">
              {SHARE_OPTIONS.map(opt => (
                <div key={opt.label} className="flex flex-col items-center gap-1.5">
                  <button
                    className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg hover:opacity-80 active:scale-95 transition-all"
            ...
            <div className="flex items-center gap-2 bg-[#3d3d3d] rounded-lg px-3 py-2">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                  copied ? "bg-green-600 text-white" : "bg-[#065fd4] text-white hover:bg-[#0559c0]"
                }`}
              >
      ...
      {/* dim overlay to close comments on outside click */}
      {showComments && (
